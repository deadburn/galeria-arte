...
export default function ArtistModal({ group, onClose }: Props) {
  ...
  return createPortal(
    <>
      ...
      <div
        className="animate-fade-in fixed inset-0 z-50 bg-black/90"
        onClick={onClose}
      />
      ...
      <div
        ...
        onClick={onClose}
      >
        <div className="flex min-h-full justify-center px-3 py-6 sm:px-4 sm:py-12">
          {/* Modal content – stop propagation so clicking inside doesn't close */}
          <div
            className="animate-fade-in-scale relative w-full max-w-4xl self-start rounded-2xl border border-black-deep/8 bg-white shadow-2xl shadow-black/15"
              ...
              {heroImage && (
                <div className="absolute inset-0">
                  <img
                    ...
                    className="h-full w-full select-none object-cover blur-2xl brightness-[0.25] saturate-150"
                  />
      ...
      {/* Lightbox for selected artwork – rendered outside the scroll container */}
      {selected && (
        <div
        ...
        >
          ...
          <div
            className="animate-fade-in-scale relative w-full max-w-5xl overflow-hidden rounded-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {selected.image_url && (
              <img
                ...
                draggable={false}
                onContextMenu={(e) => e.preventDefault()}
                className="max-h-[85vh] w-full select-none rounded-2xl object-contain"
              />
            )}
            <div className="absolute bottom-0 left-0 right-0 rounded-b-2xl bg-gradient-to-t from-black/80 to-transparent px-4 pb-4 pt-10 sm:px-6 sm:pb-5 sm:pt-12">
              <h3 className="font-heading text-lg text-white-off sm:text-xl">
                {selected.title}
              </h3>
